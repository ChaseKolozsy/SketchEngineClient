...
def build_function_code(func_name: str, method: str, path: str, parameters: list,
    ...
    sig_parts = ['api_key']  # Make api_key the first required parameter
    ...
    lines = [
        ...
        f"    {docstring}",
        f"    {url_line}",
        f"    headers = {{'Authorization': f'Bearer {{api_key}}'}}",  # Add auth headers
        f"    {params_code}",
        f"    response = requests.{method_lower}(url, headers=headers, params=params)",
        f"    return response\n"
    ]
    return "\n".join(lines)
...
def generate_api_client_from_openapi(openapi_spec_path: str, output_file: str = "generated_sketchengine_client.py"):
    ...
    # Add SketchEngineClient class definition
    lines.extend([
